...
export type ToZodSchema<T extends Record<string, any>> = {
    [K in keyof T]-?: IsNullable<T[K]> extends true ? ZodWithEffects<ZodNullable<ZodType<T[K]>>> : IsOptional<T[K]> extends true ? ZodWithEffects<ZodOptional<ZodType<T[K]>>> : ZodWithEffects<ZodType<T[K]>>;
};
export {};
